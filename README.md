# Loon Adblock Plugins

适用于 [Loon](https://nsloon.app/) 的去广告与体验优化插件集合。

## 插件列表

| 软件 | 功能 | 原作者 | 添加到 Loon | 源码 |
| --- | --- | --- | --- | --- |
| 中国银行 | 屏蔽广告图片和广告下发域名 | 思南, zirawell | [添加](https://www.nsloon.com/openloon/import?plugin=https%3A%2F%2Fraw.githubusercontent.com%2Faoomee%2FLoon-Adblock-Plugins%2Fmain%2Fplugins%2Fboc.plugin) | [查看](./plugins/boc.plugin) |
| 华住会 | 屏蔽开屏广告和首页弹窗 | 树 | [添加](https://www.nsloon.com/openloon/import?plugin=https%3A%2F%2Fraw.githubusercontent.com%2Faoomee%2FLoon-Adblock-Plugins%2Fmain%2Fplugins%2Fhuazhu.plugin) | [查看](./plugins/huazhu.plugin) |
| 铁路 12306 | 移除开屏广告 | 奶思 | [添加](https://www.nsloon.com/openloon/import?plugin=https%3A%2F%2Fraw.githubusercontent.com%2Faoomee%2FLoon-Adblock-Plugins%2Fmain%2Fplugins%2F12306.lpx) | [查看](./plugins/12306.lpx) |
| 高德地图 | 减少广告和推广内容 | 树先生, RuCu6 | [添加](https://www.nsloon.com/openloon/import?plugin=https%3A%2F%2Fraw.githubusercontent.com%2Faoomee%2FLoon-Adblock-Plugins%2Fmain%2Fplugins%2Famap.lpx) | [查看](./plugins/amap.lpx) |
| 招商银行 | 移除开屏广告和首页横幅 | 树先生 | [添加](https://www.nsloon.com/openloon/import?plugin=https%3A%2F%2Fraw.githubusercontent.com%2Faoomee%2FLoon-Adblock-Plugins%2Fmain%2Fplugins%2Fcmbchina.lpx) | [查看](./plugins/cmbchina.lpx) |
| 滴滴出行 | 减少广告和推广内容 | 树先生 | [添加](https://www.nsloon.com/openloon/import?plugin=https%3A%2F%2Fraw.githubusercontent.com%2Faoomee%2FLoon-Adblock-Plugins%2Fmain%2Fplugins%2Fdidi.lpx) | [查看](./plugins/didi.lpx) |
| GitHub | 解除拉取限制 | 树先生 | [添加](https://www.nsloon.com/openloon/import?plugin=https%3A%2F%2Fraw.githubusercontent.com%2Faoomee%2FLoon-Adblock-Plugins%2Fmain%2Fplugins%2Fgithub.lpx) | [查看](./plugins/github.lpx) |
| 闲鱼 | 移除广告和推荐内容 | 树先生 | [添加](https://www.nsloon.com/openloon/import?plugin=https%3A%2F%2Fraw.githubusercontent.com%2Faoomee%2FLoon-Adblock-Plugins%2Fmain%2Fplugins%2Fgoofish.lpx) | [查看](./plugins/goofish.lpx) |
| 虎扑 | 移除广告和推广内容 | 树先生 | [添加](https://www.nsloon.com/openloon/import?plugin=https%3A%2F%2Fraw.githubusercontent.com%2Faoomee%2FLoon-Adblock-Plugins%2Fmain%2Fplugins%2Fhupu.lpx) | [查看](./plugins/hupu.lpx) |
| 京东 | 移除开屏广告 | 树先生 | [添加](https://www.nsloon.com/openloon/import?plugin=https%3A%2F%2Fraw.githubusercontent.com%2Faoomee%2FLoon-Adblock-Plugins%2Fmain%2Fplugins%2Fjd.lpx) | [查看](./plugins/jd.lpx) |
| Keep | 减少广告和推广内容 | 树先生 | [添加](https://www.nsloon.com/openloon/import?plugin=https%3A%2F%2Fraw.githubusercontent.com%2Faoomee%2FLoon-Adblock-Plugins%2Fmain%2Fplugins%2Fkeep.lpx) | [查看](./plugins/keep.lpx) |
| 雷速体育 | 移除开屏和评论区广告 | 树先生 | [添加](https://www.nsloon.com/openloon/import?plugin=https%3A%2F%2Fraw.githubusercontent.com%2Faoomee%2FLoon-Adblock-Plugins%2Fmain%2Fplugins%2Fleisu.lpx) | [查看](./plugins/leisu.lpx) |
| Soul | 移除广告 | 树先生, 怎么肥事, 奶思 | [添加](https://www.nsloon.com/openloon/import?plugin=https%3A%2F%2Fraw.githubusercontent.com%2Faoomee%2FLoon-Adblock-Plugins%2Fmain%2Fplugins%2Fsoul-new.lpx) | [查看](./plugins/soul-new.lpx) |
| 拼多多 | 减少广告和推广内容 | 树先生 | [添加](https://www.nsloon.com/openloon/import?plugin=https%3A%2F%2Fraw.githubusercontent.com%2Faoomee%2FLoon-Adblock-Plugins%2Fmain%2Fplugins%2Fpdd.lpx) | [查看](./plugins/pdd.lpx) |
| 朴朴超市 | 移除广告和推荐内容 | 树先生, 可莉 | [添加](https://www.nsloon.com/openloon/import?plugin=https%3A%2F%2Fraw.githubusercontent.com%2Faoomee%2FLoon-Adblock-Plugins%2Fmain%2Fplugins%2Fpupu.lpx) | [查看](./plugins/pupu.lpx) |
| 淘宝 | 移除开屏广告 | 树先生 | [添加](https://www.nsloon.com/openloon/import?plugin=https%3A%2F%2Fraw.githubusercontent.com%2Faoomee%2FLoon-Adblock-Plugins%2Fmain%2Fplugins%2Ftaobao.lpx) | [查看](./plugins/taobao.lpx) |
| YouTube / YouTube Music | 广告过滤和播放体验优化 | Maasea | [添加](https://www.nsloon.com/openloon/import?plugin=https%3A%2F%2Fraw.githubusercontent.com%2Faoomee%2FLoon-Adblock-Plugins%2Fmain%2Fplugins%2Fyoutube.lpx) | [查看](./plugins/youtube.lpx) |

## 使用方法

1. 在安装了 Loon 的 iPhone 或 iPad 上点击表格中的“添加”。
2. 检查插件内容后添加并启用。
3. 插件包含 MitM 时，需安装并信任 Loon 证书，同时开启 HTTPS 解密。
4. 完全退出目标 App 后重新打开。

## 来源与说明

- `12306.lpx` 至 `youtube.lpx` 收录自 [`ishowshu/loon`](https://github.com/ishowshu/loon/tree/main/plugin)，作者信息保留在首页和插件文件中。
- 转载插件的相关权利归原作者所有；本仓库根目录的 MIT License 仅适用于自行编写的内容。
- 部分插件会加载其他 GitHub 仓库中的远程 JavaScript，安装前请自行检查脚本来源。
- App 接口可能随版本更新而变化。如规则失效，可提交 Issue，并附上脱敏后的请求 URL。
- 请勿向任何人提供密码、验证码、Cookie、Token 或完整抓包内容。
